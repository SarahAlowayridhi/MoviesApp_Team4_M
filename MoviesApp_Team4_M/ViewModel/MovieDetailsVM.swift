...
struct MovieDetails: View {
    ...
    private var header: some View {
        ZStack(alignment: .top) {
            AsyncImage(url: vm.posterURL ?? URL(string: fallbackPosterURL ?? "")) { phase in
                switch phase {
    ...
    private var topBar: some View {
        ...
        return VStack(spacing: 0) {
            HStack {
                ...
                circleIcon(system: vm.bookmarkSystemName) {
                    Task { await vm.saveMovieToSaved(movieRecordId: recordId) }
                }
        ...
        HStack(spacing: 1) {
            ForEach(0..<5, id: \.self) { i in
                Image(systemName: i < count ? "star.fill" : "star")
                    ...
                    .foregroundStyle(Color.yellow1.opacity(i < count ? 0.9 : 0.25))
            }
        }
        .padding(.leading, 8)
    }
